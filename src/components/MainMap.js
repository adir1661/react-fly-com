...
const useCahcedReportsForView = false;

function assignValuesToWindow(props) {
    ...
    let singleEmail = (reportObj, email, name) => {
        ...
        window.$.ajax({
            url: Constants.EMAIL_URL + 'email',
            method: 'POST',
            data: JSON.stringify(dataObj),
            dataType: 'json',
            headers: {
                'Authorization': window.sessionStorage.getItem(Constants.JWT_TOKEN_ID),
            },
            success: (result) => {
    ...
    const emailList = ['adir1551@gmail.com'];
    window.distributeEmails = (reportObj, emailList) => {
        emailList.forEach(email => {
            singleEmail(reportObj, email.email, email.name);
        })
    };
    window.singleEmail = singleEmail;
    window.fetchData = (url, options, payload) => {
        let body = undefined;
        if (options.method === 'GET' && payload) {
            url = new URL(url);
            Object.keys(payload).forEach(key => url.searchParams.append(key, payload[key]));
        } else if (options.method && payload) {
            options.body = payload;
        }
        if (options.body && payload) {
            return Promise.reject('error, payload declared twice!');
            try {
        ...
        let token_id = window.sessionStorage.getItem(Constants.JWT_TOKEN_ID);
        if (token_id || true) {
            console.log(token_id ? token_id.slice(0, 50) + '...' : 'no token');
            return fetch(url, {
                ...options,
                body,
                contentType: 'json',
                cache: 'no-cache',
                headers: {
                    'Authorization': token_id ? token_id.replace('\n', '').replace(' ', '') : undefined,
                    ...options.headers,
                }
            }).then(res => res.json())
                .catch(err => {
    ...
    let moveToLocation = (lat, lng, zoom = 12) => {
    ...
    window.submitFormListener = (formElement, _this, url = 'sites') => {
        formElement.on('submit', (ev) => {
            ...
            let ajaxCall = (formDetailsObject) => {
                $.ajax({
                    url: Constants.REST_API_URL + url,
                    method: "POST",
    ...
    window.heroMap = function (_latitude, _longitude, element, markerTarget, sidebarResultTarget, showMarkerLabels, mapDefaultZoom) {
        ...
        function loadData(url, ajaxData, method = 'POST') {
            ...
            let error = function (e) {
                console.log('error fatching at loadData function', e);
            };
        ...
        if ((element) !== null) {
            ...
            if (optimizedDatabaseLoading === 1) {//todo: implement lazy loading for map content.
                window.google.maps.event.addListener(window.map, 'idle', function () {
                    if (window.searchClicked !== 1) {
                        ...
                        loadData(Constants.REST_API_URL + "locations", ajaxData, "GET");
                    }
                });
            }
            else {
                window.google.maps.event.addListenerOnce(window.map, 'idle', function () {
                    loadData(Constants.REST_API_URL + "locations", null, "GET");
                });
            }
            ...
            $("[data-ajax-response='map']").on("click", function (e) {//search form on map click listener....
                ...
                if (locationValue) {
                    ...
                    window.moveToLocation(locationResult.lat, locationResult.lng, 10);
                    return;
                }
                ...
                if (categoryResult.length > 0 && categoryValue) {
                    ...
                    window.moveToLocation(latitude, longitude, 14);
                    nextIndexSite++;
                    return;
                }
            });
...
class MainMap extends Component {
    ...
    componentDidMount = () => {
        assignValuesToWindow(this.props);
